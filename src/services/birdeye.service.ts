...
class BirdEyeService {
      ...
      if (data.data?.items && Array.isArray(data.data.items)) {
        return data.data.items.map((item: Record<string, unknown>) => {
          ...
          return {
            ...
            liquidity: (item.liquidity as number) || 0,
            volume24h: (item.volume24h as number) || 0,
            trade24h: (item.trade24h as number) || 0,
            uniqueWallet24h: (item.uniqueWallet24h as number) || 0,
            base: {
              address: (base?.address as string) || "",
              symbol: (base?.symbol as string) || "",
              decimals: (base?.decimals as number) || 9,
              icon: base?.icon as string | undefined,
            },
            quote: {
              address: (quote?.address as string) || "",
              symbol: (quote?.symbol as string) || "",
              decimals: (quote?.decimals as number) || 9,
              icon: quote?.icon as string | undefined,
            },
  ...
  async getHighOpportunityTokens(limit: number = 3): Promise<TrendingToken[]> {
    try {
      ...
      return trending
        .filter(
          (token) => token.liquidity >= 100000 && token.volume24h >= 50000
        )
        .slice(0, limit);
    } catch (error) {
