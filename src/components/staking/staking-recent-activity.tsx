        ...
        <BaseCard className="p-8 flex flex-col gap-4 w-full mt-6">
            ...
            </div>
            {
                recentData.map((activity, index) => {
                    return (
                        <BaseCard className="p-5 flex justify-between">
                            <div className="flex gap-2">
                                <div className={`flex items-center justify-center rounded-full w-8 h-8 bg-[${colors.bg[index % 3]}]`}>
                                    {activity.status === 'Pending' ? <AiOutlineMinus size={12} /> : <AiOutlinePlus size={12} />}
                                </div>
                            ...
                            <div className="flex flex-col gap-1">
                                <div className="flex flex-col gap-1">
                                    ...
                                    <div className="flex gap-1 items-center">
                                        <div className={`${activity.status === 'Pending' ? 'text-secondary' : 'text-green-500'} text-xs font-normal`}>
                                            {activity.status}
                                        </div>
