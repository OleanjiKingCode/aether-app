...
const SetPriceAlertModal = ({ isOpen, setIsOpen }: SetPriceAlertModalProps) => {
    ...
    const selectMenu = ["Price goes above", "Price falls below", "Price reaches exactly"]
                ...
                <div className="flex flex-col gap-4">
                    <div className="flex gap-2">
                        <div className="flex flex-col gap-2">
                            <div className="text-foreground text-lg font-semibold">Set Price Alert for AETH</div>
                            <div className="text-xs text-muted-foreground"> Get notified when AETH reaches your target price</div>
                        </div>
                    </div>
                    <div className="flex flex-col gap-1">
                        ...
                        <Dropdown data={selectMenu} value={selectedAlertType} onChange={setSelectedAlertType} />
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <input className="border border-input p-2 text-sm outline-0" value={targetPrice} onChange={(e: any) => setTargetPrice(e.target.value)} />
                    </div>
                    <div className="flex flex-col gap-1">
                    ...
                    <div className="w-full flex items-end justify-between md:justify-end gap-3.5">
                        <div className="border border-secondary flex px-4 py-2 gap-1.5 justify-center text-muted-foreground text-sm font-medium w-full md:w-fit" onClick={() => setIsOpen(false)}>
                            Cancel
                        </div>
