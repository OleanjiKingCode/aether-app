...
const NetworkSelect = ({ data, value, onChange, className }: NetworkSelectProps) => {
        ...
        <div className={`relative w-full md:w-[220px] ${className || ""}`}>
            <button
                ...
                className={`flex items-center justify-between w-full border border-input px-3 py-2 bg-transparent text-foreground text-sm ${
                    data.length === 0 ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:bg-accent'
                }`}
                ...
                <span className="flex items-center gap-2">
                    {selected && (<>
                        {(selected.logoURI || selected.icon) ? (
                            <img 
                                src={(selected.logoURI || selected.icon)} 
                                alt={selected.name} 
                                width={20} 
                                height={20} 
