...
const RecentActivity = () => {
  ...
  useEffect(() => {
    const fetchActivity = async () => {
      const walletAddress =
        activeWalletType === "solana" ? publicKey?.toBase58() : evmAddress;
        ...
        const formattedData = result.data.map((activity: any) => {
          ...
          if (action.model === "swap") {
            ...
            if (action.transferType?.includes("VAULT")) {
              ...
              type = "Transfer";
              description = `${
                action.transferType?.includes("IN") ? "Received" : "Sent"
              } ${action.transferAsset?.symbol || "tokens"}`;
            }
  ...
  return (
    <div className="p-6 border-input border-[1px] bg-card w-full max-w-250 xl:max-w-full ">
      <div className="flex flex-col items-start gap-1.5">
                  ...
                  <span className="absolute left-2 top-1/2 -translate-y-1/2 text-muted-foreground pointer-events-none">
                    <svg width="16" height="16" fill="none" viewBox="0 0 24 24">
                      <path
                        ...
                        d="M21 21l-4.35-4.35m1.35-4.15a7 7 0 1 1-14 0 7 7 0 0 1 14 0Z"
                      />
                    </svg>
                  </span>
                  <input
        ...
        <div className="font-geist-mono font-normal text-sm text-muted-foreground">
      ...
      {!isConnected && (
        <div className="flex flex-col gap-5 w-full items-center justify-center text-center mt-10">
          ...
          <div className="flex flex-col gap-1.5 w-full items-center justify-center text-center">
            ...
            <div className="text-sm text-muted-foreground font-geist-mono max-w-[833px]">
              Once you connect your wallet and start trading, you'll see your
              recent swaps, staking activities, and cross-chain bridges here in
              a comprehensive table format.
            </div>
          </div>
          <div>
      ...
      {isConnected && (
        <div className="flex w-full flex-col mt-6">
              ...
              <tbody>
                {isLoading ? (
                  ...
                  activities.map((row, idx) => (
                    <tr
                      key={idx}
                      className="text-sm border-input border-t-[1px]"
                    >
                      <td className="px-3 py-2">
                        <div className="flex gap-2 items-center justify-start">
                          <Image
                            src={`/icon/recent-${
                              row.type === "Transfer" ? "Bridge" : row.type
                            }-icon.svg`}
                            alt={row.type}
                            width={16}
                            height={16}
          ...
          <div className="flex items-center justify-between w-full mt-4">
            <div className="text-xs text-muted-foreground font-geist-mono">
              0 of 5 row(s) selected.
            </div>
            <div className="flex gap-2">
              <button className="px-3 py-1 cursor-pointer rounded border border-input text-sm text-muted-foreground bg-background hover:bg-muted transition-colors">
                Previous
              </button>
              <button className="px-3 py-1 rounded border border-input text-sm text-muted-foreground bg-background hover:bg-muted transition-colors">
                Next
              </button>
            </div>
          </div>
